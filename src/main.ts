...
const url_careerdesign =
  "https://www.kyoto-art.ac.jp/student/careerdesign/news/?paged=1";
...
const sheet_careerdesign: GoogleAppsScript.Spreadsheet.Sheet = spreadsheet.getSheetByName(
  "careerdesign"
);
...
//キャリアデザインについて
////URLから指定範囲をスクレイピング
function get_careerdesign(): string {
  const responce: string[] = UrlFetchApp.fetch(url_careerdesign)
    .getContentText()
    .split(/\r\n|\r|\n/);
  const start_num: number = responce.indexOf(
    '          <section class="news-sect">'
...
////get_careerdesignから記事のURLを取得し配列に格納
function get_careerdesign_url(): string[] {
...
////get_careerdesignから記事のタイトルを取得し配列に格納
function get_careerdesign_title(): string[] {
  const news: string = get_careerdesign();
  const title: string[] = XmlService.parse(
    "<d>" +
      String(
...
function get_careerdesign_date(): string[] {
  const news: string = get_careerdesign();
  const date: string[] = String(news.match(/<p class="date font-roboto">.*/g))
    .replace(html_tag, "")
    .replace(/\./g, "/")
...
////sheetに書き込み
function writing_sheet_careerdesign(): void {
  const url: string[] = get_careerdesign_url();
  const date: string[] = get_careerdesign_date();
  const title: string[] = get_careerdesign_title();
  const info: string[][] = [];
  for (let i = 0; i < url.length; i = (i + 1) | 0) {
    info[i] = [title[i], url[i], date[i]];
  }
  const rows: number = info.length;
  const cols: number = info[0].length;
  const last_row: number = sheet_careerdesign.getLastRow();
  if (last_row === 0) {
    sheet_careerdesign.getRange(1, 1, rows, cols).setValues(info);
    CacheService.getScriptCache().put(
      "careerdesign_data",
      JSON.stringify(info),
      21600
    ...
    const cache: GoogleAppsScript.Cache.Cache = CacheService.getScriptCache();
    const careerdesign_data: string = cache.get("careerdesign_data");
    if (careerdesign_data == null) {
      const range: string[][] = sheet_careerdesign
        .getRange(1, 1, last_row, 3)
        .getValues();
      cache.put("careerdesign_data", JSON.stringify(range), 21600);
    }
    const data: any = JSON.parse(cache.get("careerdesign_data"));
    const info_url: string[] = [];
    const data_url: string[] = [];
    for (let i = 0; i < info.length; i = (i + 1) | 0) {
      info_url[i] = info[i][1];
    ...
    if (url_diff.length > 0) {
      ...
      if (result.length < 200) {
        sheet_careerdesign
          .getRange(1, 1, result.length, cols)
          .setValues(result);
        cache.remove("careerdesign_data");
        cache.put("careerdesign_data", JSON.stringify(result), 21600);
      } else {
        const max_result: string[][] = result.slice(0, 200);
        sheet_scholarship
          .getRange(1, 1, max_result.length, cols)
          .setValues(max_result);
        cache.remove("careerdesign_data");
        cache.put("careerdesign_data", JSON.stringify(max_result), 21600);
      }
    } else {
      cache.remove("careerdesign_data");
      cache.put("careerdesign_data", JSON.stringify(data), 21600);
    }
  }
}
////行データを取得
